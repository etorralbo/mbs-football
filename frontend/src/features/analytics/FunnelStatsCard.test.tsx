...
describe('FunnelStatsCard', () => {
  ...
  it('renders pending invites count on success', () => {
    mockUseFunnelStats.mockReturnValue({
      status: 'success',
      data: { team_created: 1, invite_created: 5, invite_accepted: 2, template_created_ai: 1, assignment_created: 2, session_first_log_added: 4, session_completed: 5 },
    })

    render(<FunnelStatsCard />)

    expect(screen.getByLabelText('Team activity')).toBeInTheDocument()
    // pending = invite_created (5) - invite_accepted (2) = 3

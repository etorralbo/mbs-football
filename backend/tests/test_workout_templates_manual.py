...
class TestBlockItems:
    ...
    # ------------------------------------------------------------------
    # Set-level idempotency and delete tests
    # ------------------------------------------------------------------

    def test_delete_set_reduces_count(
        self, client: TestClient, mock_jwt, coach_a: UserProfile
    ):
        """PATCH item replacing 3 default sets with 2 returns 200 with 2 sets."""
        mock_jwt(str(coach_a.supabase_user_id))
        ex = _create_exercise(client)
        t = _create_template(client)
        b = _create_block(client, t["id"])
        ...
        assert len(item["sets"]) == 3  # default

        r = client.patch(
            f"{BLOCK_ITEMS_URL}/{item['id']}",
            headers=HEADERS,
            json={"sets": [
                {"order": 0, "reps": 8, "weight": 80.0, "rpe": None},
                {"order": 1, "reps": 6, "weight": 85.0, "rpe": None},
            ]},
        )
        assert r.status_code == 200
        sets = r.json()["sets"]
    ...
    def test_delete_set_idempotent(
        self, client: TestClient, mock_jwt, coach_a: UserProfile
    ):
        """PATCHing with the same sets array twice is idempotent (both return 200)."""
        mock_jwt(str(coach_a.supabase_user_id))
        ex = _create_exercise(client)
        t = _create_template(client)
        b = _create_block(client, t["id"])
        ...
        payload = {"sets": [{"order": 0, "reps": 5, "weight": 100.0, "rpe": 8.0}]}
        r1 = client.patch(f"{BLOCK_ITEMS_URL}/{item['id']}", headers=HEADERS, json=payload)
        assert r1.status_code == 200
        assert len(r1.json()["sets"]) == 1

        r2 = client.patch(f"{BLOCK_ITEMS_URL}/{item['id']}", headers=HEADERS, json=payload)
        assert r2.status_code == 200
        assert len(r2.json()["sets"]) == 1

    def test_delete_item_idempotent(
        self, client: TestClient, mock_jwt, coach_a: UserProfile
    ):
        """DELETE same item twice — both calls return 204 (idempotent)."""
        mock_jwt(str(coach_a.supabase_user_id))
        ex = _create_exercise(client)
        t = _create_template(client)
        b = _create_block(client, t["id"])
        ...
        r1 = client.delete(f"{BLOCK_ITEMS_URL}/{item['id']}", headers=HEADERS)
        assert r1.status_code == 204

        r2 = client.delete(f"{BLOCK_ITEMS_URL}/{item['id']}", headers=HEADERS)
        assert r2.status_code == 204

    def test_delete_item_other_team_still_returns_404(
        self,
        client: TestClient,
        mock_jwt,
        coach_a: UserProfile,
        ...
        """IDOR protection: deleting another team's item returns 404, not 204."""
        mock_jwt(str(coach_a.supabase_user_id))
        ex = _create_exercise(client)
        t = _create_template(client)
        b = _create_block(client, t["id"])
        ...
        # coach_b must not be able to delete coach_a's item
        mock_jwt(str(coach_b.supabase_user_id))
        r = client.delete(f"{BLOCK_ITEMS_URL}/{item['id']}", headers=HEADERS)
        assert r.status_code == 404

        # Item must still exist for coach_a
        mock_jwt(str(coach_a.supabase_user_id))
        detail = client.get(f"{TEMPLATES_URL}/{t['id']}", headers=HEADERS).json()
        item_ids = [i["id"] for bl in detail["blocks"] for i in bl["items"]]
